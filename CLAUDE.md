...
- Text: `#2D3530`
- Blue Light label uses `<span className="toggle-label">` so globals.css can hide it on small screens. In Dhivehi the label is `'ނޫރަސް™'` (a coined term — see CREDITS.md) AND the span needs `fontFamily: isRtl ? "'Faruma','MV Waheed',Tahoma,sans-serif" : "'DM Sans',sans-serif"` — without the override the span inherits the parent button's DM Sans, which has no Dhivehi glyphs and falls back to whatever ugly system font the OS happens to have. The English label stays "Blue Light Blocker" — no ™ on the English side because the coined word is the Dhivehi one. Source order is `'ނޫރަސް™'` so the bidi algorithm renders ™ visually on the LEFT of the word in RTL (which is the trailing position in RTL reading order — same role ™ plays on the right side of an English word).
